...
public interface ProfileIdRepository extends Neo4jRepository<ProfileId, String> {
	...
	@Query("MATCH (p:ProfileId)-[r:employeeOf]->(c:Company) WHERE sum(toInteger(r.duration)) >= {year} RETURN p")
	public List<ProfileId> getByExperince(int year);
	
	@Query("MATCH (p:ProfileId)-[:hasSkill]->(s:Skill),(p)-[r:employeeOf]->(),(p)-[:livesIn]->(c:City) WHERE p.profileId IN {profileId} RETURN p.profileId,s.name,sum(toInteger(r.duration)),c.city")
	public List<Result> getResultSet(List<ProfileId> profileId);
